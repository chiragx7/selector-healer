...
**Scope, deliberately conservative:** `isUnreachable` fires only on a *hard* page-load failure with no successful scan (`!scannedOk && scanFailed`). An `authFailed` variant was prototyped (treat a thrown `globalSetup`/login as unreachable even when a page still 200s) but **reverted** after review: it over-reached - a failed login would flag *every* not-found selector "unreachable, try again", masking genuine removals on public pages. It also added little, because when login genuinely fails, **verify** already marks the authed selectors `page-load-failed` (not `broken`), so heal never heals them; `authFailed`'s only marginal effect overlapped the redirect-to-login case, which is exactly where the over-reach lived. `globalSetup` is still wrapped in try/catch (a failed login must not throw out of the whole heal - it degrades to unreachable via the failed page scans), and the failed-load `catch` now closes its page so a run full of failures doesn't leak one handle each.
**Residual limitations (page-load granularity):** `scannedOk` means "reached *a* loaded page", not "reached the page the element lives on". So three narrow cases still read `'removed'`: (a) a protected route that 200s to a login screen; (b) an element whose real page is a phase-2 page that fails while its phase-1 contextHint loaded; (c) an SPA that loads but hasn't rendered the element yet (the 5s `networkidle` settle mitigates this). Distinguishing them needs per-selector "did we reach *its* page" tracking; deferred. The common, clear case - a hard page-load/navigation failure - is covered honestly.
