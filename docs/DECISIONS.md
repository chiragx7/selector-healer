...
**Bounds check:** the nudge is capped at ±0.1, so a structurally-best candidate (structural ≥ the auto-apply floor of 0.8, hence nudged ≥ 0.7) can never be dropped by the relative-ratio trim behind any reachable top — the two guards compose without a hole.
