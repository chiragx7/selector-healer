...
**Alternatives considered:** (1) Ephemeral hide (this run only) — reappears on every watch save, defeating the point. (2) Permanent ignore via an inline `// selector-healer-ignore` comment — stronger but edits the user's source and never re-surfaces; kept as a possible future option. (3) Key by `id` only — a `getByRole` name edit would stay wrongly dismissed.
