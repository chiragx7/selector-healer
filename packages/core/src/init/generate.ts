...
/**
 * How to store adaptive-learning feedback, chosen at setup:
 * - `'local'` - per-developer, in the editor's workspace storage (nothing committed)
 * - `'committed'` - shared with the team via `.selector-healer/feedback.json`
 * - `'off'` - no learning
 */
export type LearningChoice = 'committed' | 'local' | 'off';
 ...
 * @param learning - the setup-time storage choice for accept/reject learning; when
 *   omitted, a documented comment is emitted and the runtime default (`'local'`) applies
 * @returns the filename to write and its contents
 *
 * @example
 * const { filename, content } = renderConfigFile(detectProjectConfig(cwd), 'local');
 * writeFileSync(filename, content);
 */
export function renderConfigFile(
  detection: ProjectDetection,
  learning?: LearningChoice,
): GeneratedConfig {
  return detection.usesTypeScript ? renderTs(detection, learning) : renderCjs(detection, learning);
}

/**
 * The `learning: {...}` config lines. With a choice made (the setup picker), emit an
 * active field; without one (CLI default), emit a documented comment - the runtime
 * default is `'local'`, so no committed file appears unless the user opts in.
 */
function learningLines(learning?: LearningChoice): string {
  if (learning === 'committed') {
    return [
      '  // Adaptive learning: share accept/reject feedback with your team.',
      "  learning: { store: 'committed' }, // commits .selector-healer/feedback.json",
    ].join('\n');
  }
  if (learning === 'local') {
    return [
      '  // Adaptive learning: keep accept/reject feedback on this machine only.',
      "  learning: { store: 'local' },",
    ].join('\n');
  }
  if (learning === 'off') {
    return ['  // Adaptive learning is turned off.', '  learning: { enabled: false },'].join('\n');
  }
  return [
    '  // Adaptive learning nudges suggestions toward the fix kinds you accept.',
    "  // Default (unset) is 'local' - per-developer, nothing committed. To share",
    '  // it with your team, uncomment:',
    "  //   learning: { store: 'committed' },  // commits .selector-healer/feedback.json",
    '  // Or turn it off:  learning: { enabled: false },',
  ].join('\n');
}
